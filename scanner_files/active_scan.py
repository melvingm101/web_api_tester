...
def scan_url(target_url):
    ...
    try:
        ...
        if is_sqli_vuln:
            print_error(f"[CRITICAL] Possible SQL Injection detected in {target_url}")
        else:
            print_success(f"[SAFE] No simple SQL errors in {target_url}")
    ...
    if "=" in target_url:
        try:
            ...
            if xss_payload in response.text:
                print_error(f"[HIGH] Possible Reflected XSS detected in {target_url}")
            else:
                print_success(f"[SAFE] No XSS reflected in {target_url}")
